module New.Encode exposing (encoder, toBytes)
...
encodeCharacters a b c d =
    {- Performance notes
       ...
       This may not normally matter but this function is called a lot so even small increases in efficiency are
       noticable

       Secondly, we combine two `uint8` into one `uint16 BE`. This has no direct speed benefit
       (elm/bytes uses a DataView, which only natively supports adding/reading uint8) but having fewer list items decreases # of encoding steps and allocation,
                    ...
                    _ ->
                        if isValidChar c then
                            let
                                ...
                                n =
                                    Bitwise.or (Bitwise.shiftLeftBy 18 n1) (Bitwise.or (Bitwise.shiftLeftBy 12 n2) (Bitwise.shiftLeftBy 6 n3))
